...
Частини 1–6 **виміряли** класичні техніки на Haiku 4.5 — малій моделі без примусового reasoning. Але фронтир 2026 — це **reasoning-моделі** (Claude Sonnet 5 / Opus 5.5 / Fable 5.1, OpenAI GPT-6, DeepSeek R1/V4), які думають самі. Ця частина — про те, **що з класики живе, що померло, і що прийшло на заміну**, за офіційними доксами обох лабораторій та дослідженнями.
...
- **Anthropic** (актуальний довідник під Sonnet 5, Opus 5.5 і Fable 5.1): *«Prefer general instructions over prescriptive steps. A prompt like "think thoroughly" often produces better reasoning than a hand-written step-by-step plan»*. Ручний CoT з тегами `<thinking>/<answer>` — тепер лише **fallback**, коли adaptive thinking вимкнено.
...
1. **Нативна схема на рівні API** (100% коректність гарантує сервер) — коли доступна. На Claude це `output_config.format`, і він працює навіть на Haiku 4.5.
...
- Claude 4.7 і новіші (Sonnet 5, Opus 5/5.5, Fable 5/5.1): нестандартні `temperature`/`top_p`/`top_k` → **400**. OpenAI: у GPT-5 і GPT-6 Astra цих параметрів немає, а GPT-6 Sol і Luna приймають їх лише з `reasoning_effort: none`. Стиль і варіативність задаються промптом, глибина — effort.
...
- **Чутливість reasoning-моделей до промпта**: KAIST ([arXiv:2505.17225](https://arxiv.org/abs/2505.17225)) показує «reasoning rigidity» — моделі **переїжджають явні умови** звичними траєкторіями (три модуси: interpretation overload, input distrust, partial instruction attention). Частину збоїв формулювання не лікує. Єдиної картини «більш чи менш чутливі» в літературі поки немає — чесно кажемо: відкрите питання.
...
1. **9-A.** Твій старий промпт для GPT-4-класу починався з «Let's think step by step» і мав 5 few-shot прикладів. Переносиш на Claude Opus 5.5. Що зміниш і чому?
2. **9-B.** Потрібен гарантовано коректний JSON на (а) Fable 5.1, (б) локальній Qwen через llama.cpp. Який механізм формату обереш для кожної і хто в кожному разі гарантує схему?

### Відповіді
1. **9-A.** Прибрати CoT-заклинання (Opus 5.5 думає завжди; глибину задає `effort`, типово `medium`), few-shot скоротити або лишити тільки якщо вони задають **формат** (не «навчають міркувати»); переписати як goal+constraints+формат виводу. І прибрати `temperature`, якщо був: на Opus 5.5 нестандартне значення дає 400.
...
- [OpenAI — Model guidance (GPT-6 Astra, Sol, Luna)](https://developers.openai.com/api/docs/guides/latest-model)
